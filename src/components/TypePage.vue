<template>
  <!-- Componente genérico para páginas de tipo (Fogo / Água / Planta) -->
  <div class="type-page">
    <!-- Header temático -->
    ...
    <div class="search-wrap">
      ...
      <div class="gen-filter">
        <button
          ...
          :key="g"
          class="gen-btn"
          :class="{ active: activeGen === g }"
          :style="activeGen === g ? { background: accent, borderColor: accent } : { borderColor: accent + '55', color: accent }"
          @click="toggleGen(g)"
        >
          {{ g }}
        </button>
...
export default {
  ...
  props: {
    ...
    accent: String,
    glow: String,
    pokemon: Array   // lista de pokémon deste tipo
  },
  data() {
    return {
      search: '',
  ...
  computed: {
    // Gerações disponíveis extraídas dos dados
    generations() {
      const set = new Set(this.pokemon.map(p => p.gen))
      return [...set].sort()
    },
    // Pokémon filtrados por busca e geração
    filtered() {
